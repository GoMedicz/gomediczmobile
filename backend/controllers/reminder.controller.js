...
const AddNewReminder = async(req, res, next) => {
  ...
  let unique = Math.random().toString(36).substring(2, 9)
      ...
      return res.send({type:'success', message:'Reminder successfully Added', status:200})
    ...
    const getUserReminder = async(req, res, next) => {
      ...
      sequelize.query(
        "SELECT id, code, pill_name, day, time from tbl_reminders where user_code =? ",
        {
