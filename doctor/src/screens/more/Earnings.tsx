...
import { FormatNumber, getBritishDate, getData, getDays, getTime } from '../../components/globalFunction';
...
const fetchTransaction = async()=>{
  ...
  let url = ServerUrl+'/api/get/doctor/withdrawal/'+wallet
  try{
...
const fetchRevenue = async()=>{
  ...
  let final =   "SELECT  SUM(amount) as revenue  from tbl_doctor_payments where doctor_code ='"+code+"'"
...
const fetchChart = async()=>{
  ...
  const start = new Date(getLastSixdays())
      ...
      const code = await getData('code');
      const wallet = await getData('wallet');
      let sqlString = result.map(data=>"sum(CAST(case when TO_CHAR(date_paid, 'yyyy-mm-dd')  = '"+data+"'  then amount::varchar else 0::varchar end AS INTEGER)) AS "+getDays(data).toLowerCase()+"")

      let final =   "SELECT  "+String(sqlString)+ " from tbl_doctor_payments where doctor_code ='"+code+"'"
